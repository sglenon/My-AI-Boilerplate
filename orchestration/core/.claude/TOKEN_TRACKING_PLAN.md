...
### 1. Parser Script
**Path:** `<project-root>/.claude/scripts/parse_usage.py`

**Modes:**
- `--transcript <path>` — Parse single transcript (SessionEnd hook)
- `--reconcile` — Scan all transcripts; parse those whose session_id is absent from the sidecar ledger (`.parsed_sessions.json`) or whose mtime has advanced since the ledger recorded it. Not gated on CSV mtime. (SessionStart hook)
...
**Path:** `<project-root>/.claude/hooks/session_end.sh`
...
### 3. Initialize CSV
**Path:** `<project-root>/.claude/usage.csv`
...
**Preserve existing:** Existing project hooks must not be clobbered. The core
baseline includes `SessionStart` and `SessionEnd`.
...
| Transcript unchanged since last parse | Skip on reconcile if mtime has not advanced since sidecar ledger timestamp |
...
| Transcripts | `~/.claude/projects/<encoded-project-path>/*.jsonl` | Input (immutable); slug derived by `resolve_transcripts_glob()` or via `--project-dir` CLI flag |

"""Shared helpers for Spec-Driven Python scripts."""
...
def resolve_specify_init_dir() -> Path:
    ...
    if not (init_root / ".specify").is_dir():
        print(
            "ERROR: SPECIFY_INIT_DIR is not a Spec-Driven project "
...
def format_spec_driven_command(command_name: str, repo_root: Path) -> str:
    separator = get_invoke_separator(repo_root)
    name = command_name.lstrip("/")
    for prefix in ("spec-driven.", "spec-driven-", "speckit.", "speckit-"):
        if name.startswith(prefix):
            name = name[len(prefix) :]
            break
    name = name.replace(".", separator)
    return f"$spec-driven{separator}{name}"

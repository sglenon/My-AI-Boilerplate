...
def main(argv: list[str] | None = None) -> int:
    ...
    if not paths.impl_plan.is_file():
        ...
        print(
            f"Run {format_spec_driven_command('plan', paths.repo_root)} first to create the implementation plan.",
            file=sys.stderr,
        )
        return 1
    ...
    if not paths.feature_spec.is_file():
        ...
        print(
            f"Run {format_spec_driven_command('specify', paths.repo_root)} first to create the feature structure.",
            file=sys.stderr,
        )
        return 1
    ...
    if tasks_template_content is None:
        ...
        print(
            ...
            ".specify/templates/overrides/tasks-template.md, or run "
            "$spec-driven-init to restore the bundled core "

---
name: spec-driven-clarify
description: "Identify underspecified areas in the current feature spec by asking up to 5 highly targeted clarification questions and encoding answers back into the spec. Use when the user invokes $spec-driven-clarify or requests the clarify phase of a spec-driven workflow."
---

## Runtime bootstrap

Before any other step, resolve the sibling `spec-driven-init` skill directory and run its bundled initializer from the repository root:

```bash
python3 <spec-driven-init-directory>/scripts/init_project.py --project-root .
```

Parse the JSON result and continue only when `ready` is `true`. The initializer preserves existing project files and supplies missing scripts/templates without installing or invoking the external Specify CLI. If the sibling skill is unavailable, stop and tell the user the complete Spec-Driven skill suite must be copied together.
...
- When constructing command invocations from hook command names, replace dots (`.`) with hyphens (`-`). For example, `spec-driven.git.commit` → `$spec-driven-git-commit`.
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    ```
    After emitting the block above you MUST actually invoke the hook and wait for it to finish before continuing. Run it the same way you would run the command yourself in this agent/session (the invocation may differ from the literal `{command}` id shown above, e.g. a skills-mode agent runs it as `/skill:spec-driven-...` or `$spec-driven-...`). Emitting the block alone does not run the hook.
...
Note: This clarification workflow is expected to run (and be completed) BEFORE invoking `$spec-driven-plan`. If the user explicitly states they are skipping clarification (e.g., exploratory spike), you may proceed, but must warn that downstream rework risk increases.
...
1. Run `python3 .specify/scripts/python/check_prerequisites.py --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run `$spec-driven-specify` or verify feature branch environment.
...
5. Sequential questioning loop (interactive):
    ...
    - **Question writing quality (applies to every question, MC or short-answer):**
       ...
       - Use everyday wording; introduce jargon only if defined in the same sentence. Self-check: a reader who does not know Spec-Driven must be able to answer from the Question line alone. Terse is fine; cryptic labels are not.
...
- If spec file missing, instruct user to run `$spec-driven-specify` first (do not create a new spec here).
...
- If any Outstanding or Deferred remain, recommend whether to proceed to `$spec-driven-plan` or run `$spec-driven-clarify` again later post-plan.

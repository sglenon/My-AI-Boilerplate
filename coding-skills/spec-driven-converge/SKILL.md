---
name: spec-driven-converge
description: "Assess the current codebase against the feature's spec, plan, and tasks, then append any remaining unbuilt work as new tasks to tasks.md so implement can complete it. Use when the user invokes $spec-driven-converge or requests the converge phase of a spec-driven workflow."
---
...
`$spec-driven-implement` can complete it. This command MUST run only after
`$spec-driven-implement` has run on the current `tasks.md`, and after `$spec-driven-tasks` has produced a complete `tasks.md`.
...
prerequisite command to run (`$spec-driven-specify` for a missing spec, `$spec-driven-plan` for a missing plan,
`$spec-driven-tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `$spec-driven-implement` to complete them; note that a follow-up converge
